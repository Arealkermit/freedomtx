...
int convertLogicalSource_130_to_131(int source)
{
...
void convertMixSource_130_to_131(int source, int *new_source)
{
...
int convertInputSource_130_to_131(int source)
{
...
void convertModelData_130_to_131(Conversion_130::ModelData_v130 &oldModel, ModelData &g_model)
{
  int new_source;
  ...
  memcpy(&g_model.header, &oldModel.header, sizeof(oldModel.header));
  ...
  memcpy(&g_model.beepANACenter, &oldModel.beepANACenter, sizeof(oldModel.beepANACenter));
  ...
  for (int i = 0; i < MAX_SPECIAL_FUNCTIONS; i++) {
    ...
    memcpy(&g_model.customFn[i].play.name, &oldModel.customFn[i].play.name, sizeof(oldModel.customFn[i].play.name));
  ...
  memcpy(&g_model.moduleData, &oldModel.moduleData, sizeof(oldModel.moduleData));
  ...
  for (int i = 0; i < MAX_SCRIPTS; i++)
  {
    ...
    {
      g_model.scriptsData[i].inputs[input].source = convertLogicalSource_130_to_131(oldModel.scriptsData[i].inputs[input].source);
    }
  }
  ...
  for (int i = 0; i < MAX_TELEMETRY_SENSORS; i++) {
    ...
    memcpy(g_model.telemetrySensors[i].label, oldModel.telemetrySensors[i].label, sizeof(g_model.telemetrySensors[i].label));
    ...
    memcpy(g_model.telemetrySensors[i].calc.sources, oldModel.telemetrySensors[i].calc.sources, sizeof(g_model.telemetrySensors[i].calc.sources));
    g_model.telemetrySensors[i].consumption.source = oldModel.telemetrySensors[i].consumption.source;
    memcpy(g_model.telemetrySensors[i].consumption.spare, oldModel.telemetrySensors[i].consumption.spare, sizeof(g_model.telemetrySensors[i].consumption.spare));
  ...
  for (int i = 0; i < MAX_TELEMETRY_SCREENS; i++)
  {
    ...
    if (screenType == TELEMETRY_SCREEN_TYPE_BARS)
    {
      ...
      for (int j = 0; j < 4; j++)
      {
        g_model.screens[i].bars[j].source = convertLogicalSource_130_to_131(oldModel.screens[i].bars[j].source);
    ...
    else if (screenType == TELEMETRY_SCREEN_TYPE_VALUES)
    {
      ...
      for (int line = 0; line < 4; line++)
      {
        ...
        {
          g_model.screens[i].lines[line].sources[k] = convertLogicalSource_130_to_131(oldModel.screens[i].lines[line].sources[k]);
        }
      }
    }
  ...
  {
    g_model.expoData[i].srcRaw = convertInputSource_130_to_131(oldModel.expoData[i].srcRaw);
  }

  for (int i = 0; i < MAX_MIXERS; i++)
  {
    convertMixSource_130_to_131(g_model.mixData[i].srcRaw, &new_source);
    g_model.mixData[i].srcRaw = new_source;
  }

  for (int i = 0; i < MAX_LOGICAL_SWITCHES; i++)
  {
    g_model.logicalSw[i].v1 = convertLogicalSource_130_to_131(oldModel.logicalSw[i].v1);
  }
...
void convertRadio_130_131()
{
  ...
  g_eeGeneral.spare1 = oldRadio.spare1;
  ...
  for (int i = 0; i < MAX_SPECIAL_FUNCTIONS; i++) {
    ...
    memcpy(&g_eeGeneral.customFn[i].play.name, &oldRadio.customFn[i].play.name, sizeof(oldRadio.customFn[i].play.name));
  ...
  g_eeGeneral.enableRotaryInverse = oldRadio.enableRotaryInverse;
  ...
  for (std::list<ModelsCategory *>::const_iterator it = cats.begin(); it != cats.end(); ++it, ++index) {
    ...
    for (uint8_t i = 0; i < (*it)->size(); i++) {
      ...
      if (error) {
        ...
        //convert model data
        convertModelData_130_to_131(oldModel, g_model);
