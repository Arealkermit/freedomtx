...
CrossfireFrameStatus crsfFrameStatus = {0};
  ...
  switch(id) {
    ...
    case COMMAND_ID:
    {
      if (telemetryRxBuffer[3] == 0xEA    // radio address
        ...
        if (getCrossfireTelemetryValue<7>(1, value, module))
          crsfFrameStatus.portID = value;
        if (getCrossfireTelemetryValue<8>(4, value, module)) {
          for (uint8_t i = 0; i < sizeof(CROSSFIRE_BAUDRATES); i++) {
            if ((uint32_t)value == CROSSFIRE_BAUDRATES[i]) {
              crsfFrameStatus.baudIndex = i;
              break;
            }
          }
        }
        moduleState[module].counter = CRSF_FRAME_SPEED_PROPOSAL;
    ...
    case RADIO_ID:
      if (telemetryRxBuffer[3] == 0xEA    // radio address
        ...
        if (getCrossfireTelemetryValue<4>(6, (int32_t&)update_interval, module) && getCrossfireTelemetryValue<4>(10, offset, module)) {
          ...
          getModuleSyncStatus(module).update(update_interval, offset + SAFE_SYNC_LAG);
        }
      }
      break;
